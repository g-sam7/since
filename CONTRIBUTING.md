...
- **Dates and times**: `date-fns` (with `@date-fns/utc` and `@date-fns/tz`) is the default for all date handling in the frontend and shared domain code. Do not hand-roll date arithmetic or formatting, and do not add other date libraries (dayjs, luxon, moment). Call the library directly; do not write wrappers that only rename or re-export a date-fns function. A helper that adds its own logic around a date-fns call (choosing which date to measure from, advancing a recurrence) is acceptable code.
...
- **Security**: Before changing framework entry points, middleware, auth, cookies or error handling, check which framework defaults the change replaces, since protections can disappear silently. Call out any weakened protection in the PR and cover it with a regression test. Verify these changes in the dev server too, because some security warnings only appear there.
- **Server functions**: Require authentication for any private data, and take ownership fields from the session, never from client input. Validate input on the server even when the client already does. Return expected failures as typed errors, and never send raw server or database errors to the client.
- **Testing Strategy**:
